...
export const RewardIcons = new FilesCollection({
  debug: false,
  ...
  onAfterUpload(image) {
    ...
    Object.keys(image.versions).forEach(versionName => {
      ...
      gm(fs.createReadStream(image.versions[versionName].path), image.name).crop(dims.width, dims.height, dims.left, dims.top).resize("480", "480").stream().pipe(writeStream);
