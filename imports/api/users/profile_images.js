...
export const ProfileImages = new FilesCollection({
  ...
  onAfterUpload(image) {
    Object.keys(image.versions).forEach(versionName => {
      ...
      var dims = image.meta;
      gm(fs.createReadStream(image.versions[versionName].path), image.name).crop(dims.width, dims.height, dims.left, dims.top).resize("640", "640").stream().pipe(writeStream);
